...
function get20sEmail(arr) {
  ...
  const people20s = arr.filter(({age}) => age >= 20 && age < 30);
  const sortingByAge = (a,b) => a['age'] - b['age'];
  const getEmails = ({email}) => email;

  return people20s.sort(sortingByAge).map(getEmails) || [];
}
